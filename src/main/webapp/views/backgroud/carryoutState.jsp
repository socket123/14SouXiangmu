            ...
            <div class="form-group">
                <div class="col-xs-12">
                    <div class="row">
                        <!-- 一组开始 -->
                        <div class="col-xs-5">
                            <label  class="col-xs-4 control-label">状态名称:</label>
                            <div class="col-xs-8 ">
                                <input type="text" class="form-control" name="name" id="name" value="${carryoutState.name }">
                            </div>
                        </div>
                        <!-- 一组结束 -->
                        <div class="col-xs-2">
...
<script type="text/javascript">
    $(document).ready(function(){
        $(".search").on("click",function(){
            location.href = "${url }admin/carryoutState?"+$("#searchForm").serialize();
        });
    });
    ...
    $('#page-jq').jqPaginator({
        ...
        onPageChange:function(num,type){
            if(type == 'change'){
                ...
                var name=$("#name").val();
                window.location.href="${url }admin/carryoutState?pageNumber="+num+"&name="+name;
            }
        }
    });

    function addCarryoutType(){
        window.location.href="${url}admin/carryoutState/gotoCarryoutStateAdd";
    }


    function edit(id){
        window.location.href="${url}admin/carryoutState/gotoEdit?id="+id;
    }

    function deleteCarryoutType(id){
        $.ajax({
            url : '${url }admin/carryoutState/deleteCarryoutState',
            type:'post',
            data:{id:id},
            success:function(result)
            {
                if(result.message==1){
                    layer.alert("载具状态删除成功!",function(){
